...
// 定数定義
// ==============================
//※※　DIGITSという名前の定数or定義で値は４
#define DIGITS 4    // 問題の桁数
...
//※※　整数型の変数cursor
int cursor;    // 選択カーソル
//※※　整数型、配列の要素数DIGITSのtarget
int target[DIGITS];    // 目標数の保存配列
//※※　整数型、配列の要素数DIGITSのnum
int num[DIGITS];	// 推測数の保存配列
	...
	for (int i = 0; i < DIGITS; i++)    //※※
	{
		...
		do {
			...
			for (int j = 0; j < i; j++)
			{
				...
				{
					// 今回の数字が無効な値だったら isValid を false に。有効な値だったら isValid を true にする
					isValid = false;
				}
			}
