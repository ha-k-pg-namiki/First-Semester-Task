...
int CrateGraphHandle[NUMBER_OF_CRATE_TYPES];		// クレートの種類最大数 ※※
...
void DrawInit()
{
	...
	GroundGraphHandle = -1;
	BlockGraphHandle = -1;
	for( int i = 0; i < NUMBER_OF_CRATE_TYPES; i++ )		// ※※
	{
...
void DrawObject( int x, int y, int type )
{
	...
	// マスの座標を、画面座標へ変換
	PlayerGraphHandle[STAGE_WIDTH][STAGE_HEIGHT];
	// 地面の描画
	PlayerGraphHandle[x][y] = DrawGraph(x,y, GroundGraphHandle,TRUE);
	// 配置物の描画
	PlayerGraphHandle[x][y] = DrawGraph(x, y, BlockGraphHandle, TRUE);
}
